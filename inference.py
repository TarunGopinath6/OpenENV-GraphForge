#!/usr/bin/env python3
"""
Inference Script — GraphForge AST Mutation Agent
=================================================
MANDATORY
...
STDOUT FORMAT
  ...
  Rules:
    ...
    - error is the last action error string, or null if none.
    - All fields on a single line with no newlines within a line.
    - Score is in [0, 1].

SCORING
    score = 0.5 * code_correctness + 0.5 * sequence_match

    code_correctness:
        Fraction of ADD/MODIFY mutations whose submitted code is syntactically
        valid Python (ast.parse does not raise SyntaxError).

    sequence_match:
        Compares the agent's mutation sequence against the reference answer
        stored in llm_result.actions of the query JSON.  For each expected
        action, +0.5 if the mutation type matches (ADD/MODIFY/DELETE) and
        +0.5 if the node_id matches.  Normalised over the number of expected
        actions.
"""
...
def log_info(msg: str) -> None:
    """Extra terminal logging — not part of the mandatory format."""
...
def compute_code_correctness(mutations: List[dict]) -> float:
    """Fraction of ADD/MODIFY mutations with syntactically valid Python code."""
...
def compute_sequence_score(mutations: List[dict], expected_actions: List[dict]) -> float:
    """
    Compare agent mutations against the reference llm_result.actions.

    For each expected action at position i:
      +0.5 if mutation[i].type  matches expected[i].type
      +0.5 if mutation[i].node_id matches expected[i].node_id
    Score is normalised over the number of expected actions.
    If no reference exists, returns 1.0 (no penalty).
    """
...
def compute_score(
    ...
    """
    Returns (total_score, code_score, seq_score) each in [0, 1].

    total_score = 0.5 * code_score + 0.5 * seq_score
    """
...
def parse_action(text: str) -> Optional[ASTAction]:
    ...
    The naive regex approach (r"\\{[^{}]*\\}") breaks whenever the `code` field
    contains Python braces (dicts, f-strings, sets).  This scanner walks the
    string character-by-character, respecting string literals and escape
    sequences, so it correctly identifies the outermost JSON object regardless
    of nested braces in the payload.
    """
    # Fast path: model returned clean JSON with no leading/trailing text
    stripped = text.strip()
    try:
    ...
    # Slow path: find the outermost balanced { … } in the response
    start = stripped.find("{")
    if start == -1:
        return None
...
def run_task(
    ...
    log_info(f"log_file : {obs.metadata.get('log_path', 'none')}")
    log_divider()
    ...
    try:
        ...
        log_info(f"  code_correctness = {code_score:.2f}  "
        ...
        log_info(f"  sequence_match   = {seq_score:.2f}  "
...
def main() -> None:
    ...
    for task_name, seed in tasks:
        try:
            scores[task_name] = run_task(env, client, task_name, seed)
        except Exception as exc:
    ...
    print(f"  {'Average':<22} {'█' * int(avg * 20) + '░' * (20 - int(avg * 20))}  {avg:.4f}",
          flush=True)
    log_divider("=")
